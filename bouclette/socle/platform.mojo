...
Everything above `bouclette.socle` — `bouclette.error`, `bouclette.handle`,
`bouclette.ctypes`, `bouclette.watch` — imports its OS-level names from here
and never from `bouclette.socle.<os>`. Porting bouclette to a second OS is
then an edit of this file plus a new `bouclette.socle.<os>` package, not a
sweep through every portable module.

`bouclette.drivers` is exempt: its members are named after the mechanism
...
comptime answer for code that needs to branch, and `bouclette.socle`
...
syscalls `bouclette.net` is built on, and the coroutine stack backend.
...
- **needs a per-OS equivalent** — Linux-shaped, with no drop-in twin
  ...
  `bouclette.socle.<os>` that presents the same signature, not a rename.
...
- **portable by contract** — not identical in nature across platforms,
  but bouclette adopts one backend's encoding as the library-wide
...
from bouclette.socle import is_linux, is_darwin, is_windows
...
from bouclette.socle.linux.errno import Errno
...
from bouclette.socle.linux.raw.ctypes import (
    c_void,
    c_char,
    c_int,
    c_uint,
...
# `socklen_t`. The portable message layer (`bouclette.net.message`, added
# with the message futures) and `bouclette.watch` build these structs
...
from bouclette.socle.linux.raw.utils import _to_be

# --- Socket syscalls (portable, except `_raw_accept4`) ----------------
#
# Thin wrappers that raise on a negative return; `bouclette.net.socket`
...
from bouclette.socle.linux.net.syscalls import (
    _bind,
    _connect,
    _fcntl_getfl,
    _fcntl_setfl,
...
#
# `bouclette.net.options` hardcodes the portable enums and asserts them
...
# `bouclette.socle.windows`.

from bouclette.socle.linux.ucontext_stack import _UcontextStack
...
from bouclette.socle.linux.thread import _Mutex, _Condvar, _Thread
