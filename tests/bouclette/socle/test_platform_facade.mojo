...
`bouclette/socle/platform.mojo` is the single seam between the public API
...
seam, instead of scattered across `bouclette/error.mojo`, `bouclette/handle.mojo`,
`bouclette/ctypes/`, `bouclette/net/`, `bouclette/coroutine/` and `bouclette/watch/`.

It also pins the invariant the seam exists for: no module above
`bouclette.socle` may name an OS package directly.
"""
...
from bouclette.socle.platform import (
    platform_name,
    # Errno vocabulary — bouclette/error.mojo, bouclette/watch/outcome.mojo.
    Errno,
    EACCES,
    EADDRINUSE,
    EADDRNOTAVAIL,
    ...
    EPIPE,
    ETIMEDOUT,
    # Completion flag encoding — bouclette/proactor/completion.mojo.
    IORING_CQE_BUFFER_SHIFT,
    IORING_CQE_F_BUFFER,
    IORING_CQE_F_MORE,
    # Raw handles — bouclette/handle.mojo, bouclette/watch/accept.mojo.
    UnsafeFd,
    close,
    close_unchecked,
    unsafe_fd_as_arg,
    # C scalars — bouclette/ctypes/__init__.mojo.
    c_void,
    c_char,
    c_int,
    c_uint,
    ...
    c_size_t,
    c_ssize_t,
    # Address layouts and byte order — bouclette/net/addr.mojo.
    __be32,
    sockaddr_in,
    sockaddr_in6,
    socklen_t,
    _to_be,
    # Socket syscalls — bouclette/net/socket.mojo.
    _socket,
    _bind,
    _listen,
    _raw_accept4,
    ...
    _getsockname,
    _getpeername,
    # Socket option constants — bouclette/net/options.mojo, socket.mojo.
    AF_UNSPEC,
    AF_UNIX,
    AF_INET,
    AF_INET6,
    ...
    O_CLOEXEC,
    MSG_NOSIGNAL,
    # Coroutine stacks — bouclette/coroutine/_stack.mojo.
    _UcontextStack,
)
...
def test_completion_flag_encoding_is_reachable() raises:
    """`bouclette/proactor/completion.mojo` decodes flags with these bits."""
...
def test_address_layouts_are_reachable() raises:
    """`bouclette/net/addr.mojo` builds its storage types out of these."""
...
def test_socket_syscalls_are_reachable() raises:
    """The wrappers `bouclette/net/socket.mojo` wraps in `IOError` resolve."""
...
def test_socket_option_constants_are_reachable() raises:
    """The values `bouclette/net/options.mojo` and `socket.mojo` assert against."""
...
def test_coroutine_stack_is_reachable() raises:
    """`bouclette/coroutine/_stack.mojo` aliases `_CoroStack` to this."""
