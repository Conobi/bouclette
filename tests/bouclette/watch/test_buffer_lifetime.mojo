...
from bouclette.watch import WatchLoop, RecvFuture, SendFuture
...
def test_result_hands_the_buffers_back() raises:
    ...
    """
    var pair = _Pair()
    var msg = String("hello bouclette")
...
def test_future_dropped_before_run() raises:
    ...
    var loop = WatchLoop()
    var send_f = loop.send(pair.client, _bytes(String("hello bouclette")))
    var recv_f = loop.recv(pair.peer, List[UInt8](length=32, fill=0))
    _ = send_f^
    _ = recv_f^
...
def test_future_dropped_in_a_call_that_returns_before_run() raises:
    ...
    var loop = WatchLoop()
    _drop_send(loop.send(pair.client, _bytes(String("hello bouclette"))))
