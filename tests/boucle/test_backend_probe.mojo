...
def test_auto_selects_correct_backend() raises:
    ...
    var cl = CompletionLoop(sq_entries=4)
...
def test_forced_epoll_always_works() raises:
    ...
    var cl = CompletionLoop(sq_entries=4, backend=Backend.EPOLL)
    assert_true(cl.backend() is Backend.EPOLL)

    var wl = WatchLoop(sq_entries=4, backend=Backend.EPOLL)
...
def test_forced_io_uring_behavior() raises:
    ...
    if _has_io_uring():
        var cl = CompletionLoop(sq_entries=4, backend=Backend.IO_URING)
